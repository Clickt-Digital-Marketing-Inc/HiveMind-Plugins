...
### 2. Pick lanes
Choose **≤ N disjoint lanes** (N = concurrency cap, default 5, project-overridable). Disjoint means no two lanes own the same file, page, schema region, or other contended surface. Where light contention is unavoidable, manage it explicitly: assign insertion anchors / regions per lane and brief both executors on the boundary. Exclude issues sharing a surface with an in-flight lane — they run next round. Decision-gated issues are not lanes; they are escalations. A lane must not run live mutation tests on records a sibling lane may be reading; where unavoidable, sentinel values carry lane attribution.
...
Update the state file (see `/checkpoint`), then run the **learnings loop** (below) — `/lessons-review` is the interactive mechanism when the user is present.
...
- Before treating any board state as anomalous, check the issue for a wrap-up comment and the state file's git history for checkpoints this session didn't write — In Review + wrap-up = delivered work, possibly by a parallel session.
- A shared live tree/DB supports exactly one orchestrator unless the user rules otherwise.
- Check integration branch names against `git ls-remote` before use.
