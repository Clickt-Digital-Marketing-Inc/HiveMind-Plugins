#!/usr/bin/env python3
"""Performance report — multi-format deliverable builder (thin CLI).

Reads a findings JSON (schema authoritative in `references/performance-report.md`)
and emits the standard analytical bundle via the shared toolkit (`_shared/render`):
md (narrative + full per-campaign table), html (interactive ROAS-goal explorer),
and xlsx (formula-driven, LibreOffice-normalized).

Usage:
    python3 build_perf_report.py --input findings.json --outdir artifacts \\
...
SKILL_NAME = "google-ads-performance-reporting"


def main() -> int:
    ap = argparse.ArgumentParser(description="Build the performance-report bundle.")
    ...
    for p in written:
        print(f"  - {p.name}")
    print(f"Spend={rmodel.money(s['spend'], cur)}  Revenue={rmodel.money(s['revenue'], cur)}  "
          f"ROAS={s['roas']}  Scale={s['scale']}  Fix={s['fix']}  campaigns={s['campaigns']}")
    return 0
