#!/usr/bin/env python3
"""Build / check the Performance Max momentum-filter workbook (.xlsx) — thin wrapper.

The workbook is rendered by the shared toolkit (`_shared/render/xlsx.py`) from the
pmax model (`pmax_core`) and the xlsx layout (`pmax_xlsx_spec`). This file is just
the CLI; the dependency-free primaries (md, html) come from build_pmax_filter.py.

Sheets: Controls (tunable thresholds + self-rewriting logic + live COUNTIF/SUMIF) ·
Campaign trends (every campaign + Status; active rows carry formulas referencing the
Controls cells) · Sensitivity (static snapshot).

Excel-compatibility: the file is normalized through LibreOffice (`soffice`) by
default so it opens in Excel; if soffice is missing the build FAILS (exit 2). Use
--no-normalize to override. Real-Excel open is not verified here.

Usage:
    python3 build_pmax_workbook.py --input findings.json \\
        --output "pmax-momentum-acme-2026-06-27.xlsx" --brand "Acme Corp"
    python3 build_pmax_workbook.py --check --input <file>.xlsx
...
import pmax_xlsx_spec as xspec        # noqa: E402
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build / check the Performance Max momentum-filter .xlsx.")
    ...
    print(f"  universe={stats['universe']}  active={stats['scored']}  no_activity={stats['no_activity']}")
    print(f"  at current params -> Block1={stats['block1']}  Block2={stats['block2']}  "
          f"winner_spend={stats['winners_spend']}  loser_spend={stats['losers_spend']}")
