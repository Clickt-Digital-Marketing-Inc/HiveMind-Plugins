#!/usr/bin/env python3
"""Build / check the conversions & tracking workbook (.xlsx) — thin wrapper.

The workbook is rendered by the shared toolkit (`_shared/render/xlsx.py`) from
the conv_tracking model (`conv_tracking_core`) and the xlsx layout
(`conv_tracking_xlsx_spec`). This file is just the CLI; the dependency-free
primaries (md, html) come from build_conv_tracking_report.py.

Usage:
    python3 build_conv_tracking_workbook.py --input findings.json \\
        --output "conv-tracking-acme-2026-07-12.xlsx" --brand "Acme Corp"
    python3 build_conv_tracking_workbook.py --check --input <file>.xlsx
...
import conv_tracking_xlsx_spec as xspec      # noqa: E402
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build / check the conversions & tracking .xlsx.")
    ...
    print(f"  campaigns={stats['campaigns']}  scored={stats['scored']}  no_benchmark={stats['no_benchmark']}")
    print(f"  at current params -> Critical={stats['critical']}  High={stats['high']}  "
          f"Watch={stats['watch']}  Clean={stats['clean']}")
    print(f"  config: flagged={stats['config_flagged']}/{stats['config_actions']}  "
          f"no_primary_action={stats['config_no_primary_action']}")
