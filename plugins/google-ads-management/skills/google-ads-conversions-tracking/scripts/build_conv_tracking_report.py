#!/usr/bin/env python3
"""Conversions & tracking advisor — multi-format deliverable builder (thin CLI).

Reads a findings JSON (schema is authoritative in
`references/conversion-tracking-filter.md`) and emits the standard analytical
bundle via the shared render toolkit (`_shared/render`):

  md   — narrative report (provenance, headline, config-health checklist,
         manual EC/Consent-Mode checks, CVR-drop sensitivity table, the
         excluded-campaign list, the full per-campaign trend table). Zero deps.
  html — a self-contained interactive explorer: sliders over the trend params
         + a live sensitivity strip + the static config-health/manual-check
         panels, recomputing live in any browser. No install, no cloud.
  xlsx — the tunable Controls + Campaign-trend + Snapshot workbook (needs
         openpyxl; LibreOffice-normalized so it opens in Excel).

All formats share one classification engine (conv_tracking_core), so they can
never disagree. The render config lives in conv_tracking_spec.

Usage:
    python3 build_conv_tracking_report.py --input findings.json --outdir artifacts \\
...
sys.path.insert(0, str(HERE))            # conv_tracking_core / conv_tracking_spec
...
SKILL_NAME = "google-ads-conversions-tracking"


def main() -> int:
    ap = argparse.ArgumentParser(description="Build the conversions & tracking advisor deliverable bundle.")
    ...
    if "xlsx" in formats:
        import conv_tracking_xlsx_spec as xspec  # stdlib data; openpyxl stays inside render.xlsx
    ...
    for p in written:
        print(f"  - {p.name}")
    print(f"Trend: Critical={s['critical']} High={s['high']} Watch={s['watch']} "
          f"Clean={s['clean']} no-benchmark={s['no_benchmark']} (of {s['campaigns']} campaigns)")
    print(f"Config health: {s['config_flagged']} flagged of {s['config_actions']} actions"
          + ("  — NO PRIMARY CONVERSION ACTION" if s["config_no_primary_action"] else ""))
    print(f"Manual (EC/Consent Mode): {s['manual_user_confirmed']} confirmed via CSV, "
          f"{s['manual_not_confirmed']} not confirmed")
    return 0
