#!/usr/bin/env python3
"""Bidding-strategy Data Maturity Score — multi-format deliverable builder (thin CLI).

Reads a findings JSON (schema is authoritative in
`references/bidding-strategy-maturity.md`) and emits the standard analytical
bundle via the shared render toolkit (`_shared/render`):

  md   — narrative report (provenance, headline, gate sensitivity, borderline
         campaigns, excluded campaigns). Zero deps.
  html — a self-contained interactive explorer: sliders + gate-sensitivity
         strip, recomputing live in any browser. No install, no cloud, no
         Excel. The interactive primary.
  xlsx — the tunable Controls + Live-filter + Snapshot workbook (needs
         openpyxl; LibreOffice-normalized so it opens in Excel).

All formats share one classification engine (bidding_core), so they can never
disagree. The render config lives in bidding_spec.

Usage:
    python3 build_bidding_report.py --input findings.json --outdir artifacts \\
...
sys.path.insert(0, str(HERE))            # bidding_core / bidding_spec
...
SKILL_NAME = "google-ads-bidding-strategy"


def main() -> int:
    ap = argparse.ArgumentParser(description="Build the bidding-strategy Data Maturity Score bundle.")
    ...
    if not (findings.get("meta") or {}).get("reconciliation"):
        sys.stderr.write(
            ...
            "UNVERIFIED. Assemble findings from the saved raw pull / CSV export with "
    ...
    if "xlsx" in formats:
        import bidding_xlsx_spec as xspec  # stdlib data; openpyxl stays inside render.xlsx
    ...
    for p in written:
        print(f"  - {p.name}")
    print(f"Over-automated(under-data)={s['over_automated_under_data']}  "
          f"Over-automated={s['over_automated']}  Under-automated={s['under_automated']}  "
          f"avg maturity={s['avg_maturity_score']:.2f}  "
          f"critical spend={rmodel.money(s['critical_spend'], cur)}  "
          f"universe={s['universe']} (scored {s['scored']}, no-spend {s['no_spend']}, "
          f"unsupported {s['unsupported_strategy']})")
    return 0
