...
The single entry point for the Google Ads skill suite. It presents an interactive task menu,
collects what the chosen task needs (account or input files, date window, output formats, context),
and routes to the right skill. Use an inline widget when the current host supports one; otherwise
use the conversational fallback below. It runs nothing itself and queries nothing beyond listing
...
1. If `mcp__visualize__read_me` and `show_widget` are available, call
   `mcp__visualize__read_me({modules:['interactive']})` once before the first widget. If either tool
   is unavailable, skip widget preparation and use **Fallback — no widget tool available**.
...
Keep your own response text short (one line, e.g. "Pick a task below"). When a widget is available,
the interactive UI lives there; otherwise the conversational fallback carries the same choices.
...
**A. Tunable skill** (`tunable: true`) — render the **interactive tuner** when supported; build no
files yet.
- **Bundle skills** (the 10 google-ads-management tuners) emit a widget JSON, then assemble it:
  ```bash
  python3 "${PLUGIN_ROOT}/skills/<skill>/scripts/build_<prefix>_report.py" \
      --input <findings>.json --formats "" --brand "{brand}" --emit-widget <tmp>/widget.json
  python3 "${PLUGIN_ROOT}/skills/google-ads/references/build_widget.py" \
...
- **cm3-by-product** is bespoke (its explorer is `cm3_html.py`, not `_shared`): its builder emits the
  **assembled fragment directly** — no `build_widget.py` step. Invoke the installed
  `cm3-profitability` plugin's `cm3-by-product-report` skill and let that skill resolve its own
  plugin root before it runs:
  ```bash
  cm3_by_product.py --csv <shopping.csv> [--cogs-csv <shopify.csv>] \
      --brand "{brand}" --emit-widget <tmp>/widget.html
  ```
  Never derive one installed plugin's location from another plugin's root; marketplace caches
  isolate each plugin by name and version.
...
If the host cannot render the tuner, present the current model KPIs and adjustable parameters in
text, accept parameter changes conversationally, and offer the same artifact actions from Step 5.

**B. Up-front skill** (`formats_upfront: true` — the audit): the route prompt already named the
formats, so **build them now** by invoking the installed `google-ads-audit` plugin's
`google-ads-audit` skill. Let that skill resolve and run its own bundled builder into `artifacts/`,
then render
...
- **Tunable bundle skills** (quality-score, budget-pacing, keywords-search-terms, performance-reporting,
  ...
  `${PLUGIN_ROOT}/skills/<skill>/scripts/build_<prefix>_report.py
...
- **cm3-by-product** (tunable; own builder): rebuild via
  the installed `cm3-profitability` plugin's `cm3-by-product-report` skill using
  `--csv <shopping.csv> [--cogs-csv <shopify.csv>]` plus the tuned params the Outputs button passed
...
- **mediametrics / audit**: invoke the selected installed plugin's skill and let that skill resolve
  and run its **own** bundled builder per its SKILL.md — bespoke single-output flags, **not**
  `--input/--formats`. They rebuild from their original inputs (CSV exports / findings). If the
  selected plugin is not installed, say so and stop that route instead of guessing a sibling path.
- **Reduced-bundle advisory skills** (account-health, audience-targeting — md+xlsx, no html/tuner):
  rebuild via `${PLUGIN_ROOT}/skills/<skill>/scripts/build_<prefix>_report.py --input <findings>.json
...
`python3 "${PLUGIN_ROOT}/skills/google-ads/references/resolve_vault.py"`
...
If the host cannot render an inline widget, do not fail and do not require a host-specific question
tool. Use this conversational fallback:
1. Present the four areas (Management / Audit / Profitability / MediaMetrics) as a numbered list and
   ask the user to choose one.
2. List that area's tasks from the catalog and let the user choose one by number or name.
3. Capture the inputs conversationally (account + window, or file paths; then formats + context).
4. Run the task skill, then present the model-backed summary and the same output choices as Step 5.
5. Build only the output the user chooses and return its artifact links in the conversation.
