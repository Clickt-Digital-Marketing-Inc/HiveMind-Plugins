...
def md_rows(model):
    ...
    headers = ["Campaign", "Ad Group", "Audience", "List Type", "Status", "Bid Modifier",
               f"Cost ({cur})" if cur else "Cost", "Conv.", "CPA", "CTR", "Flags", "Score", "Priority"]
    out = []
    for r in model["rows"]:
        out.append([
            r["campaign"], r["ad_group"], r["list_name"], r["list_type"], r["status"],
            f"{r['bid_modifier']:.2f}", f"{r['cost']:,.2f}", f"{r['conversions']:.2f}",
    ...
    return {
        ...
        "note": "No row loss: every applied-audience criterion in the pull appears here — "
                "scored (with flags/score/priority) or excluded (negative/exclusion criteria, "
                "never scored). Sorted by cost (highest first).",
        "headers": headers,
